...
class _NetworkGroupsListState extends State<NetworkGroupsList>
  ...
  Future<void> _loadGroups() async {
    ...
    try {
      ...
      setState(() {
        _myGroups = (myGroupsData as List)
            .map(
              (e) => NetworkCommunity.fromJson(
                e['communities'] as Map<String, dynamic>,
              ),
            )
            .toList();

        _suggestedGroups = (suggestedData as List)
            .map(
              (e) => NetworkCommunity.fromJson(
  ...
  Widget _buildGroupsList(
    List<NetworkCommunity> groups, {
    required bool isMyGroups,
  }) {
    if (groups.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(
              isMyGroups ? Icons.groups : Icons.explore,
              size: 64,
              color: Colors.grey,
            ),
  ...
  Widget _buildGroupCard(
    NetworkCommunity group, {
    required bool isMyGroups,
  }) {
    final String? bannerUrl = group.bannerUrl;
    final bool hasBanner =
        bannerUrl != null && bannerUrl.isNotEmpty;

    return GestureDetector(
      onTap: () => context.go(
        '/network/community/${group.id}',
      ),
      child: Container(
